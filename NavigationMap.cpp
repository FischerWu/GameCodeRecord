...
void NavigationMap::setAiMap(MapData* mapData)
{
    if (mapData) {
        map = mapData->getMap();
        navigationMap = mapData->getNavigationMap(); // 直接拷贝障碍物列表
        mapWidth = mapData->getMapWidth();
        mapHeight = mapData->getMapHeight();
    }
}

std::vector<std::pair<int, int>> NavigationMap::pathFinding(int startX, int startY, int endX, int endY) const
{
    ...
    for (int i = 0; i < mapHeight; i++)
    {
        ...
        for (int j = 0; j < mapWidth; j++)
        {
            if (map[i][j] != 1)
            {
                ...
                nodeMap[i][j]->hValue = (abs(endX - j) + abs(endY - i));
            }
            else
            {
                nodeMap[i][j] = nullptr;
    ...
    if (!v.empty())
    {
        ...
        while (destination)
        {
            s.push(std::pair<int, int>(destination->x, destination->y));
